...
public class AssessmentRunner {
    ...
    private final static Set<String> CORPORATE_SUPPLIERS = new HashSet<>();
    static {
        CORPORATE_SUPPLIERS.add("AVIS");
        CORPORATE_SUPPLIERS.add("BUDGET");
        CORPORATE_SUPPLIERS.add("ENTERPRISE");
        CORPORATE_SUPPLIERS.add("FIREFLY");
        CORPORATE_SUPPLIERS.add("HERTZ");
        CORPORATE_SUPPLIERS.add("SIXT");
        CORPORATE_SUPPLIERS.add("THRIFTY");
    }

    private final static char[] SIPPCODE_SORTING_ORDER = new char[] {'M', 'E', 'C'};
    ...
    public static void main(String[] args) {
        ...
        // If time allows, include an optional step to remove all FuelType.FULLFULL cars that are priced above
        // the median price within their groups.
        // Step is optional, so pass "filter" arg to use.
        boolean shouldFilter = false;
        for (String arg : args) {
            if ("filter".equalsIgnoreCase(arg)) {
                shouldFilter = true;
            }
        }


        // sort of informal attempt at a Context object... In more production like env, this would be
        // initialised with contextual/behavourial
        //here its initialised with classes to control
                // The Sorting behaviour of the data
                // The grouping behaviour of the data (used when filtering)
                // The filter criteria applicable to those groupings
                // Filtering is optional, so have a filter flag
        final Context ctx = new Context(
           new MultiComparator(new SupplierComparator(CORPORATE_SUPPLIERS), new SIPPCodeComparator(SIPPCODE_SORTING_ORDER), new RentalCostComparator()),
           new MultiComparator(new SupplierComparator(CORPORATE_SUPPLIERS), new SIPPCodeComparator(SIPPCODE_SORTING_ORDER)),
           new MultiFilter(FuelPolicyFilter.FULLFULL(), new RentalCostFilter()),
           shouldFilter
        );

        // display has a context object which controls the sorting/filtering behaviour for this invocation.
